...
namespace Cog {
	...
	*/
	...
	public:
		...
		CogAnim(string name, string sheetPath, string ref, int frames, int lines, int start,
		...
		CogAnim() : name(""), sheetPath(""), ref(""), frames(1), lines(1), start(1), end(1), increment(1), speed(1), repeat(1), isRevert(false) {
			//this->rootAnim = spt<Anim>();
			sheetInit = false;
		}
		...
		*/
		void GetParametersFromReference(CogAnim* reference) {
			...
			// insert children
			vector<spt<CogAnim>>& refChildren = reference->children;

			for (auto it = refChildren.begin(); it != refChildren.end(); ++it) {
				spt<CogAnim> child = (*it);
				AddChild(child);
			}
		}
		...
		*/
		void GetAllNodes(vector<CogAnim*> &output) {
		...
		*/
		spt<CogAnim> FindChild(string name) {
			if (this->GetName().compare(name) == 0) return spt<CogAnim>(this);

			for (auto it = children.begin(); it != children.end(); ++it) {
				if ((*it)->GetName().compare(name) == 0) return (*it);
				else {
					auto childFound = (*it)->FindChild(name);
					if (childFound != spt<CogAnim>()) return childFound;
				}
			}
		...
		*/
		bool AddChild(spt<CogAnim> child) {
		...
		*/
		bool RemoveChild(spt<CogAnim> child) {
			auto found = find(children.begin(), children.end(), child);
			if (found != children.end()) {
		...
		*/
		vector<spt<CogAnim>>& GetChildren() {
			return children;
		}
