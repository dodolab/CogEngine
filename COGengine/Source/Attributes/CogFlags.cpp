...
namespace Cog {
	...
	CogFlags::CogFlags(vector<int> states) {
		Init();
		for (size_t i = 0; i < states.size(); i++) SetState(states[i]);
	}

	CogFlags::CogFlags(int state) {
		Init();
		if (state != -1) SetState(state);
	}

	CogFlags::CogFlags(int state1, int state2) {
		Init();
		if (state1 != -1) SetState(state1);
		if (state2 != -1) SetState(state2);
	}

	CogFlags::CogFlags(int state1, int state2, int state3) {
		Init();
		if (state1 != -1) SetState(state1);
		if (state2 != -1) SetState(state2);
	...
	CogFlags::CogFlags(int state1, int state2, int state3, int state4) {
		Init();
		if (state1 != -1) SetState(state1);
		if (state2 != -1) SetState(state2);
		if (state3 != -1) SetState(state3);
	...
	CogFlags::CogFlags(int state1, int state2, int state3, int state4, int state5) {
		Init();
		if (state1 != -1) SetState(state1);
		if (state2 != -1) SetState(state2);
		if (state3 != -1) SetState(state3);
	...
	CogFlags::CogFlags(const CogFlags& obj) {
		Init();
	...
	vector<int> CogFlags::GetAllStates() const {
		vector<int> output;
	...
	bool CogFlags::HasState(int state) const {
	...
	void CogFlags::SwitchState(int state1, int state2) {
	...
	CogFlags& CogFlags::operator=(const int& st1) {
		delete otherFlags;
	...
	CogFlags& CogFlags::operator=(const CogFlags& st1) {
		flags1 = st1.flags1;
		flags2 = st1.flags2;
		flags3 = st1.flags3;
		flags4 = st1.flags4;
	...
	bool CogFlags::operator==(int st2) {
		return HasState(st2);
	}

	bool CogFlags::operator==(const CogFlags& st2) {
		if (otherFlags != nullptr) {
			// compare map
	...
	bool CogFlags::operator!=(int st1) {
		return !(*this == st1);
	}

	bool CogFlags::operator!=(const CogFlags& st1) {
		return !(*this == st1);
	}

	CogFlags CogFlags::operator+(int st1) {
		CogFlags copy(*this);
		copy.SetState(st1);
		return copy;
	}
	...
	bool CogFlags::Contains(CogFlags& other) const {
		// return first false
	...
	bool CogFlags::ContainsAtLeastOne(CogFlags& other) const {
		// return first true
		vector<int> allStates = other.GetAllStates();
		for (size_t i = 0; i < allStates.size(); i++) {
	...
	void CogFlags::DoStateOperation(bool set, int state) {
		int index = GetStateIndex(state);
		int offset = GetStateOffset(state);
		int binary = 1 << offset;
