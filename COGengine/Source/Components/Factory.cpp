...
Transform Factory::CalcTransform(Node* node, ofVec2f pos, CalcType posCalc, float scaleX, CalcType scaleCalc, ofVec2f anchor, int width, int height, Node* parent){
	ofVec2f scrSize = COGGetScreenSize();

	Transform& parentTrans = parent->GetTransform();
	Transform nodeTransform = Transform(0, 0);
