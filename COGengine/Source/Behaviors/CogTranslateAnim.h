...
namespace Cog {
	...
	*/
	class CogTranslateAnim : public CogBehavior {
	...
	public:
		...
		*/
		CogTranslateAnim(ofVec3f from, ofVec3f to, float speed, bool additive) :
		...
		*/
		CogTranslateAnim(ofVec3f from, ofVec3f to, float speed, FadeFunction fadeFunction) :
			additive(additive), to(to), from(from) {
			this->actual = ofVec3f(from);
			this->distX = to.x - from.x;
			this->distY = to.y - from.y;
