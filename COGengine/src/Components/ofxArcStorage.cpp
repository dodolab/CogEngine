...
spt<ofImage> ofxArcStorage::Get2DImage(string path){
...
spt<ofImage> ofxArcStorage::Preload2DImage(string path){
	auto found = loadedImages.find(path);
	if (found != loadedImages.end()){
		return (found->second);
	}
...
spt<ofVboMesh> ofxArcStorage::GetMesh(string path){
...
spt<ofTrueTypeFont> ofxArcStorage::GetFont(string path, int size){
	ofTrueTypeFont* font = new ofTrueTypeFont();
	font->loadFont(path, size, true, true);
	return spt<ofTrueTypeFont>(font);
}

spt<ofxAraSound> ofxArcStorage::GetSound(string path){
	auto found = loadedSounds.find(path);
	if (found != loadedSounds.end()){
		return (found->second);
	}

	ofxAraSound* snd = new ofxAraSound(path);
	return spt<ofxAraSound>(snd);
}

spt<ofxXmlSettings> ofxArcStorage::PreloadXMLFile(string path){
...
spt<ofxXmlSettings> ofxArcStorage::LoadXMLFile(string path){
	ofxXmlSettings* xml = new ofxXmlSettings();
	xml->loadFile(path);
	auto xmlPtr = spt<ofxXmlSettings>(xml);
	return xmlPtr;
...
spt<ofxAraAnim> ofxArcStorage::GetAnimation(string name){
	auto found = loadedAnimations.find(name);
	if (found != loadedAnimations.end()) return found->second;
	else return spt<ofxAraAnim>();
}

void ofxArcStorage::StoreAnimation(spt<ofxAraAnim> anim){
	MASSERT(anim->GetName().compare("") != 0, "ofxArcStorage", "Attempt to store animation without a name!");
...
spt<ofxAraSpriteSheet> ofxArcStorage::GetSpriteSheet(string name){
	auto found = loadedSpriteSheets.find(name);
	if (found != loadedSpriteSheets.end()) return found->second;
	else return spt<ofxAraSpriteSheet>();
}

void ofxArcStorage::StoreSpriteSheet(spt<ofxAraSpriteSheet> spriteSheet){
	MASSERT(spriteSheet->GetName().compare("") != 0, "ofxArcStorage", "Attempt to store spritesheet without a name!");
