...
public:
	...
	*/
	void LoadAnimations(spt<ofxXmlSettings> xml, vector<spt<ofxAraAnim>>& rootAnims){
		...
		if (xml->tagExists("animations")){
			...
			map<string, ofxAraAnim*> referencedAnims;
			...
			for (int i = 0; i < numberOfAnims; i++){
				...
				xml->pushTag("anim", i);
				// load
				spt<ofxAraAnim> anim = spt<ofxAraAnim>(CreateAnimationFromXml(xml, referencedAnims));
...
private:
	...
	*/
	ofxAraAnim* CreateAnimationFromXml(spt<ofxXmlSettings> xml, map<string, ofxAraAnim*>& referencedAnims){

		int innerAnimations = xml->getNumTags("anim");

		ofxAraAnim* anim = new ofxAraAnim();
		...
		if (anim->GetRef().length() != 0){
			...
			for (int i = 0; i < innerAnimations; i++){
				xml->pushTag("anim", i);
				spt<ofxAraAnim> newAnim = spt<ofxAraAnim>(CreateAnimationFromXml(xml, referencedAnims));
	...
	*/
	void FillBaseAttributes(spt<ofxXmlSettings> xml, ofxAraAnim* anim){
	...
	*/
	void FillOtherAttributes(spt<ofxXmlSettings> xml, ofxAraAnim* anim){
	...
	*/
	spt<ofxAraAnim> FindAnimByName(string name, vector<spt<ofxAraAnim>> anims){
		for (auto it = anims.begin(); it != anims.end(); ++it){
			spt<ofxAraAnim> anim = (*it);
			if (anim->GetName() == name) return anim;
		}

		return spt<ofxAraAnim>();
	}
	...
	*/
	void ProcessRefAnimationFromXml(spt<ofxXmlSettings> xml, map<string, ofxAraAnim*>& referencedAnims,
		int rootAnimIndex, vector<spt<ofxAraAnim>>& rootAnims){
		...
		if (ref.length() != 0){
			...
			ofxAraAnim* refAnim = referencedAnims[name];

			if (ref.find(".") == -1){
				// reference doesn't contain dot - it means that we can find its reference in THIS scope
				spt<ofxAraAnim> reference = rootAnims[rootAnimIndex]->FindChild(ref);

				if (reference != spt<ofxAraAnim>()){
					// found reference
					...
					spt<ofxAraAnim> rootReference = FindAnimByName(ref, rootAnims);
					if (rootReference == spt<ofxAraAnim>()) throw ConfigErrorException(string_format("Referenced animation %s not found", ref.c_str()));
				...
				spt<ofxAraAnim> root = FindAnimByName(rootAnimName, rootAnims);
				spt<ofxAraAnim> scopeAnim = root->FindChild(subAnim);
				if (root == spt<ofxAraAnim>() || scopeAnim == spt<ofxAraAnim>()) throw ConfigErrorException(string_format("Referenced animation %s not found", ref.c_str()));
...
public:
	...
	AnimNodeContext(spt<ofxAraAnim> node, bool isScopeReverted, bool isRootNode){
...
*/
class ofxArbAnim : public ofxAreBehavior{
private:
	// animation root
	spt<ofxAraAnim> root;
	// actual tree context
...
public:
	...
	ofxArbAnim(spt<ofxAraAnim> anim) : root(anim){

	}

	void Init(){
		if (root == spt<ofxAraAnim>()){
			COGLogError("ofxArbAnim", "Animation cant' run, entity is null");
			ended = true;
			return;
		}
	...
	virtual void Update(const uint64 delta, const uint64 absolute){
		...
		if (!Ended()){
			...
			spt<ofxAraAnim> actualNode = context.GetActualChild();
...
private:
	...
	*/
	bool TryNextFrame(spt<ofxAraAnim> actualNode){
	...
	*/
	bool TryChildren(spt<ofxAraAnim> actualNode){
	...
	*/
	bool TryNextLoop(spt<ofxAraAnim> actualNode){
		context.actualLoop++;
