...
int ofxAreNode::idCounter = 1;

ofxAreNode::ofxAreNode(string tag) : type(ObjType::OBJECT), subType(0), id(idCounter++), transform(0, 0){
	Construct();
	if (!tag.empty()) SetTag(tag);
}

ofxAreNode::ofxAreNode(ObjType type, int subType, string tag) : type(type), subType(subType), id(idCounter++), transform(0, 0){
	Construct();
	if (!tag.empty()) SetTag(tag);
}

ofxAreNode::ofxAreNode(const ofxAreNode& copy) : type(copy.type), subType(copy.subType), id(idCounter++), transform(0, 0){
	Construct();
	tag = copy.tag;
...
ofxAreNode::~ofxAreNode(){
	...
	for (list<ofxAreBehavior*>::iterator it = behaviors.begin(); it != behaviors.end(); ++it)
	{
	...
	// delete attributes
	for (map<int, ofxAreAttr*>::iterator it = attributes.begin(); it != attributes.end(); ++it){
	delete (it->second);
	}*/
...
void ofxAreNode::UpdateTransform(bool deep){
...
void ofxAreNode::Update(const uint64 delta, const uint64 absolute){
...
void ofxAreNode::Draw(const uint64 delta, const uint64 absolute){
...
bool ofxAreNode::AddBehavior(ofxAreBehavior* beh){
	behaviorsToAdd.push_back(beh);

	return true;
}

bool ofxAreNode::RemoveBehavior(ofxAreBehavior* beh, bool erase){
...
bool ofxAreNode::RemoveAttr(int key, bool erase){

	map<int, ofxAreAttr*>::iterator it = attributes.find(key);

	if (it != attributes.end()){
		ofxAreAttr* attr = it->second;
		attributes.erase(it);
...
bool ofxAreNode::HasAttr(int key) const{
	return attributes.find(key) != attributes.end();
}


void ofxAreNode::SubmitChanges(bool applyToChildren){
	InsertElementsForAdding();
	DeleteElementsForRemoving();
...
bool ofxAreNode::AddChild(ofxAreNode* child){
...
bool ofxAreNode::RemoveChild(ofxAreNode* child, bool erase){
...
bool ofxAreNode::RemoveFromParent(bool erase){
	ofxAreNode* parent = GetParent();
...
ofxAreNode* ofxAreNode::FindPredecessor(ObjType type){
	ofxAreNode* parent = this->parent;
...
string ofxAreNode::GetInfo(bool includeChildren, bool includeAttributes){
...
void ofxAreNode::GetInfo(bool includeChildren, bool includeAttributes, std::ostringstream& ss, int level){
	spaces(level * 4, ss);
...
void ofxAreNode::InsertElementsForAdding(){
...
void ofxAreNode::DeleteElementsForRemoving(){

	// delete behaviors
	for (auto it = behaviorToRemove.begin(); it != behaviorToRemove.end(); ++it){
		std::pair<ofxAreBehavior*, bool> item = (*it);
		ofxAreBehavior* beh = item.first;
	...
	// delete children
	for (auto it = childrenToRemove.begin(); it != childrenToRemove.end(); ++it){
		std::pair<ofxAreNode*, bool> item = (*it);
		ofxAreNode* child = item.first;
