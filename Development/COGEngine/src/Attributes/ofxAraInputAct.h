...
public:
	...
	*/
	ofxAraInputAct(int keyboardKey) : inputType(InputType::KEYBOARD), key(keyboardKey){
		Init(); 
	}
	...
	*/
	ofxAraInputAct(int touchId, ofVec3f pos) : inputType(InputType::TOUCH), touchId(touchId), position(pos){
		Init();
	}

	ofxAraInputAct() : touchId(-1), key(-1){
		Init();
	}
