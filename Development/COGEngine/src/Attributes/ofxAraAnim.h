...
protected:
	...
	bool isRevert;
	// list of children
	vector<spt<ofxAraAnim>> children;
...
public:
	...
	ofxAraAnim(string name, string sheetPath, string ref, int frames, int lines, int start,
		...
		sheetInit = false;
		//	this->rootAnim = spt<ofxAraAnim>();
	}

	ofxAraAnim() : name(""), sheetPath(""), ref(""), frames(1), lines(1), start(1), end(1), increment(1), speed(1), repeat(1), isRevert(false){
		//this->rootAnim = spt<ofxAraAnim>();
		sheetInit = false;
	}
	...
	*/
	void GetParametersFromReference(ofxAraAnim* reference){
		...
		// insert children
		vector<spt<ofxAraAnim>>& refChildren = reference->children;

		for (auto it = refChildren.begin(); it != refChildren.end(); ++it){
			spt<ofxAraAnim> child = (*it);
			AddChild(child);
		}
	}
	...
	*/
	void GetAllNodes(vector<ofxAraAnim*> &output){
		output.push_back(this);

		for (auto it = children.begin(); it != children.end(); ++it){
			spt<ofxAraAnim> an = (*it);
	...
	*/
	spt<ofxAraAnim> FindChild(string name){
		if (this->GetName().compare(name) == 0) return spt<ofxAraAnim>(this);

		for (auto it = children.begin(); it != children.end(); ++it){
			if ((*it)->GetName().compare(name) == 0) return (*it);
			else{
				auto childFound = (*it)->FindChild(name);
				if (childFound != spt<ofxAraAnim>()) return childFound;
			}
		}
	...
	*/
	bool AddChild(spt<ofxAraAnim> child){
	...
	*/
	bool RemoveChild(spt<ofxAraAnim> child){
		auto found = find(children.begin(), children.end(), child);
		if (found != children.end()){
	...
	*/
	vector<spt<ofxAraAnim>>& GetChildren(){
		return children;
	}
